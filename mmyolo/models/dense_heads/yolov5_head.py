...
from ..gaze_v0.box_filter_layer import FilteringLayer
...
class YOLOv5Head(BaseDenseHead):
    ...
    def __init__(self,
        ...
        self.featmap_strides = self.head_module.featmap_strides # stride=8이면, 앵커박스가 특성 맵의 8x8 크기의 영역마다 이동. 640/8 = 80
        self.num_levels = len(self.featmap_strides)  # [8, 16, 32]. num_levels는 3

        # # 추가
        # self.img_scale = train_cfg.img_scale
        # self.train_batch = train_cfg.train_batch    
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        Returns:
            ...
            predictions, and objectnesses.
        """
        # self.box_filter_layer = FilteringLayer(cfg=self.test_cfg)
        
        return self.head_module(x) # [0] -> [1,2,80,80],[1,2,40,40],[1,2,20,20] [1] -> [1,4,80,80],[1,4,40,40],[1,4,20,20]
    ...
    def predict_by_feat(self,
        ...
        """
        assert len(cls_scores) == len(bbox_preds)  #[1, 2, 80, 80],[1, 2, 40, 40],[1, 2, 20, 20] 랑 [1, 4, 80, 80],[1, 4, 40, 40],[1, 4, 20, 20] # len은 3이다.
        ...
        num_imgs = len(batch_img_metas)  # 이미지 수 1
        featmap_sizes = [cls_score.shape[2:] for cls_score in cls_scores] # [torch.Size([80, 80]), torch.Size([40, 40]), torch.Size([20, 20])]

        # If the shape does not change, use the previous mlvl_priors
        # mlvl_priors는 multi-level prior boxes(앵커 박스) 이다.
        ...
        flatten_priors = torch.cat(self.mlvl_priors) # self.mlvl_priors는 [6400,2],[1600,2],[400,2]이다. 중심좌표 x,y를 가진다. flatten_priors는 [8400,2]

        # featmap_size=[80,80]일때, featmap_size.numel()=6400, self.num_base_priors=1(각 그리드 셀에서 사용되는 앵커 박스의 수), stride=8
        mlvl_strides = [
            flatten_priors.new_full(
                (featmap_size.numel() * self.num_base_priors, ), stride) for  # 특성 맵 앵커박스의 총 개수 크기 만큼 stride 값으로 채움.
            featmap_size, stride in zip(featmap_sizes, self.featmap_strides)
        ]
        flatten_stride = torch.cat(mlvl_strides)  # mlvl_strides는 [6400],[1600],[200]. flatten_stride는 [8400]. 6400개는 8로, 1600개는 16, 200개는 32로.

        # flatten cls_scores, bbox_preds and objectness
        flatten_cls_scores = [                              # flatten_cls_scores는 [1,6400,2], [1,1600,2], [1,400,2]
            cls_score.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                  self.num_classes)  # self.num_classes 는 2이다.
            for cls_score in cls_scores                     # cls_scores는 [1,2,80,80],[1,2,40,40],[1,2,20,20]
        ]
        flatten_bbox_preds = [                              # flatten_bbox_preds는 [1,6400,4], [1,1600,4], [1,400,4]
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4)
            for bbox_pred in bbox_preds
        ]               

        flatten_cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid() # [1,8400,2]: [-14.5329, -11.6043]->[4.8803e-07, 9.1269e-06]
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
        flatten_decoded_bboxes = self.bbox_coder.decode(                    # (앵커프리) flatten_bbox_preds를 앵커 포인트(flatten_priors[None])기준으로 bbox를 계산함.
        ...
        for (bboxes, scores, objectness,
            ...
            if scores.shape[0] == 0:                        # scores.shape[0]=8400
            ...
            if cfg.multi_label is False:
                ...
                scores, labels, keep_idxs, _ = filter_scores_and_topk(  # socres가 score_thr(0.001)보다 낮은거 제거하고 상위 topk만 남음. labels=0or1. keep_idxs는 8400개 앵커들의 id.
                    scores, score_thr, nms_pre) 

            results = InstanceData(
                scores=scores, labels=labels, bboxes=bboxes[keep_idxs])  # keep_idxs로 유효한 bboxes 좌표 가져옴.
            ...
            if rescale:
                ...
                results.bboxes /= results.bboxes.new_tensor(
                    scale_factor).repeat((1, 2))    # scale_factor=(5.47008547008547, 5.47008547008547)
            ...
            # FilteringLayer를 사용하여 필터링 적용
            results = self.box_filter_layer(
                results=results,
                rescale=rescale,
                with_nms=with_nms,
                img_meta=img_meta
            ...
            # # filter 알고리즘 대신 score 기준으로 필터링
            # results_face = results[results.labels == 0]
            # results_eye = results[results.labels == 1]
            ...
            # # Get face detection with highest confidence score
            # if len(results_face) > 0:
            #     max_face_idx = results_face.scores.argmax()
            #     filtered_scores.append(results_face.scores[max_face_idx])
            #     filtered_labels.append(results_face.labels[max_face_idx])
            #     filtered_bboxes.append(results_face.bboxes[max_face_idx])

            # # Get eye detections with highest confidence scores
            # if len(results_eye) > 0:
            #     # Get indices of top 2 eye scores
            #     top_k = min(2, len(results_eye))
            #     _, top_eye_idxs = results_eye.scores.topk(top_k)
                
            #     filtered_scores.extend(results_eye.scores[top_eye_idxs])
            #     filtered_labels.extend(results_eye.labels[top_eye_idxs])
            #     filtered_bboxes.extend(results_eye.bboxes[top_eye_idxs])

            # # Convert lists to tensors and create new results
            # if filtered_scores:
            #     results = InstanceData(
            #         scores=torch.stack(filtered_scores),
            #         labels=torch.stack(filtered_labels),
            #         bboxes=torch.stack(filtered_bboxes)
            #     )
            # else:
            #     # If no detections, keep empty results
            #     results = InstanceData(
            #         scores=torch.empty(0, device=results.scores.device),
            #         labels=torch.empty(0, device=results.labels.device),
            #         bboxes=torch.empty((0, 4), device=results.bboxes.device)
            #     )

            # claming 은 주어진 범위 내에 있도록 제한하는 작업이다.
            results.bboxes[:, 0::2].clamp_(0, ori_shape[1]) # x좌표
            results.bboxes[:, 1::2].clamp_(0, ori_shape[0]) # y좌표
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: Union[list,dict]) -> dict:
        ...
        # batch_data_samples가 list인 경우
        if isinstance(batch_data_samples, list):
            losses, outs = super().loss(x, batch_data_samples)
        # batch_data_samples가 dict인 경우
        else:
            outs = self(x)            # outs[0]: 클래스 확률 outs[1]: bbox 예측 outs[2]: 객체 존재 확률
            # Fast version
            loss_inputs = outs + (batch_data_samples['bboxes_labels'], # loss_inputs[3]
                                  batch_data_samples['img_metas']) # loss_inputs[4] -> [{'batch_input_shape': torch.Size([320, 320])}]
            losses = self.loss_by_feat(*loss_inputs)

        results_list = self.extract_proposal_feat(*outs) # -> 코드 추가
    ...
    def extract_proposal_feat(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
                        objectnesses: Optional[List[Tensor]] = None, 
                        cfg: Optional[ConfigDict] = None,
        ...
        train_batch = self.train_cfg['train_batch']
        img_size = self.train_cfg['img_scale'][0]

        assert len(cls_scores) == len(bbox_preds) # cls_scores 의 len = 3, torch.Size([12, 2, 40, 40]) ... 20,20 / 10,10           
        # batch 12일 때, cls score[0] max = -4.4245 / min = -14.6961

        objectnesses = False

        cfg = self.test_cfg if cfg is None else cfg  # test cfg 
        cfg = copy.deepcopy(cfg)

        multi_label = cfg.multi_label # True
        multi_label &= self.num_classes > 1
        cfg.multi_label = multi_label # True

        num_imgs = cls_scores[0].shape[0]
        featmap_sizes = [cls_score.shape[2:] for cls_score in cls_scores] # [torch.Size([40, 40]), torch.Size([20, 20]), torch.Size([10, 10])]
        ...
        if featmap_sizes != self.featmap_sizes:
            self.mlvl_priors = self.prior_generator.grid_priors(
                featmap_sizes,
                dtype=cls_scores[0].dtype,
                # device=cls_scores[0].device) # self.mlvl_priors 의 len = 3/ torch.Size([1600, 2]), torch.Size([400, 2]), torch.Size([100, 2])
                device=self.device)
            self.featmap_sizes = featmap_sizes
        flatten_priors = torch.cat(self.mlvl_priors) # 1600+400+100 = (2100,2)

        mlvl_strides = [
            flatten_priors.new_full(
                (featmap_size.numel() * self.num_base_priors, ), stride) for
            featmap_size, stride in zip(featmap_sizes, self.featmap_strides)
        ] # mlvl_strides[0]  = torch.Size([1600]) 8로 채워짐 / [1] = [400]  16 으로 채워짐/ [2] = [100] 32로 채워짐

        flatten_stride = torch.cat(mlvl_strides) # [2100]   => X

        # flatten cls_scores, bbox_preds and objectness
        flatten_cls_scores = [
            cls_score.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                  self.num_classes) # torch.Size([1, 1600, 2]) , 400..,100..
            for cls_score in cls_scores
        ]
        flatten_bbox_preds = [
            bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) # torch.Size([1, 1600, 4]) , 400..,100..
            for bbox_pred in bbox_preds
        ]

        flatten_cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid() # torch.Size([1, 2100, 2]) -> [0] min: 4.3584e-06/ max : 0.3464
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1) # torch.Size([1, 2100, 4]) -> [0] min: 0.0664 / max: 11.9645 -> min 값 0 아래 없음
        flatten_decoded_bboxes = self.bbox_coder.decode(
            flatten_priors[None], flatten_bbox_preds, flatten_stride) # [0] min: -70.6034 

        results_list = []
        for (bboxes, scores
             ) in zip(flatten_decoded_bboxes, flatten_cls_scores
                              ): # flatten_decoded_bboxes = torch.Size([12, 2100, 4]) / flatten_cls_scores = torch.Size([12, 2100, 2]) / flatten_objectness = torch.Size([12, 134400])

            score_thr = cfg.get('score_thr', -1) # 0.001
            # yolox_style does not require the following operations

            if scores.shape[0] == 0: # pass
                empty_results = InstanceData()
                empty_results.bboxes = bboxes
                empty_results.scores = scores[:, 0]
                empty_results.labels = scores[:, 0].int()
            ...
            if cfg.multi_label is False: # pass
                scores, labels = scores.max(1, keepdim=True)
                scores, _, keep_idxs, results = filter_scores_and_topk(
                    scores,
                    score_thr,
                ...
                scores, labels, keep_idxs, _ = filter_scores_and_topk(
                    scores, score_thr, nms_pre) # nms_pre = 30000 -> 결과값 score min: 0.0010 / max : 0.3464  & keep_idxs min: 19 / max : 2099

            results = InstanceData(
                scores=scores, labels=labels, bboxes=bboxes[keep_idxs]) # results['bboxes'] min: -70.6034  / max: 379.224

            if cfg.get('yolox_style', False): # pass
                # do not need max_per_img
                cfg.max_per_img = len(results)
            ...
            # 바운딩 박스 좌표가 이미지 경계를 벗어나지 않도록 제한함
            results.bboxes[:, 0::2].clamp_(0, img_size)
            results.bboxes[:, 1::2].clamp_(0, img_size)

            results_list.append(results)
        
        return results_list # labels. bboxes.scores