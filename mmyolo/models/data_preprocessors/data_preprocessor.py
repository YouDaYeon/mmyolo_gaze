...
class YOLOv5DetDataPreprocessor(DetDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # inputs, data_samples, gaze_samples = data['inputs'], data['data_samples'], data['gaze_samples']
        inputs, data_samples, gaze_samples, ori_shapes, imgs_path = data['inputs'], data['data_samples'], data['gaze_samples'], data['ori_shapes']['ori_shapes'], data['imgs_path']['imgs_path']    ######################
        # inputs, data_samples, gaze_samples, ori_shapes, imgs_path, seg_maps_path = data['inputs'], data['data_samples'], data['gaze_samples'], data['ori_shapes']['ori_shapes'], data['imgs_path']['imgs_path'], data['seg_maps_path']['seg_maps_path']     ######################
        ...
        ori_shapes = [(ori_shapes[i]) for i in range(len(inputs))]  ######################
        imgs_path = [(imgs_path[i]) for i in range(len(inputs))]  ######################
        # seg_maps_path = [(seg_maps_path[i]) for i in range(len(inputs))]  ######################
        data_samples_output = {
            'bboxes_labels': data_samples['bboxes_labels'],
            'gazes_labels': gaze_samples['gazes_labels'],
            'img_metas': img_metas,
...
class PPYOLOEDetDataPreprocessor(DetDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # Batch random resize image. 여기 원래 했는데 config 파일에 None을 해서 안함.
        if self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                inputs, data_samples = batch_aug(batch_inputs, data_samples)

        inputs = torch.stack(batch_inputs)  # batch_augments 안하는 대신 리스트를 텐서로 변환
        if self._enable_normalize:
            inputs = (inputs - self.mean) / self.std
