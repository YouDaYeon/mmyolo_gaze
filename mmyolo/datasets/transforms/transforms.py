...
from mmdet.datasets.transforms import RandomCenterCropPad as MMDET_RandomCenterCropPad   # CenterCrop 추가
...
class RandomCenterCropPad(MMDET_RandomCenterCropPad):
    def __init__(self,
                 crop_size: Optional[tuple] = None,
                 ratios: Optional[tuple] = (0.6, 0.7, 0.8, 0.9, 1.0),
        ...
        super().__init__(
            ...
            to_rgb=to_rgb,
            test_mode=test_mode,
            test_pad_mode=test_pad_mode,
            test_pad_add_pix=test_pad_add_pix,
    ...
    def _train_aug(self, results):
        ...
        # # 최대 시도 횟수 설정 (크롭 영역 내에 객체가 포함되지 않는 경우를 방지)
        # max_attempts = 10
        ...
        # h_border = self._get_border(self.border, h)     # 이미지 경계에서 중심 선택 범위
        # w_border = self._get_border(self.border, w)

        # center_x = random.randint(low=w_border, high=w - w_border)
        # center_y = random.randint(low=h_border, high=h - h_border)
        
        # center는 항상 가운데 점으로 변경
        center_x = w // 2
        center_y = h // 2
        ...
        # bbox가 없는 경우
        if torch.all(gt_bboxes.tensor == 0):
            results['img'] = cropped_img
            results['img_shape'] = cropped_img.shape[:2]
            return results
            
        # bbox 중 패치 내에 들어있는지 확인
        mask = self._filter_boxes(patch, gt_bboxes) # bool 타입 배열 반환

        # bbox 중 하나라도 크롭 영역 밖에 있으면 아무것도 안함.
        if not mask.all():                      
            return results

        # 크롭 영역 내에 모든 bbox가 있으면 크롭 영역 이미지와 바운딩 박스 위치 보정
        results['img'] = cropped_img
        results['img_shape'] = cropped_img.shape[:2]
        ...
        gt_bboxes = gt_bboxes[mask]         # 유효한 바운딩 박스만 추출
        gt_bboxes.translate_([              # 바운딩 박스의 위치 보정
        ...
        # ignore_flags와 labels 처리
        if results.get('gt_ignore_flags', None) is not None:
            results['gt_ignore_flags'] = results['gt_ignore_flags'][mask][keep]
        if results.get('gt_bboxes_labels', None) is not None:
            results['gt_bboxes_labels'] = results['gt_bboxes_labels'][mask][keep]
        
        # max_attempts 후에도 실패하면 원본 이미지 반환
        return results
        
@TRANSFORMS.register_module()
class Float32ToUint8:
...
class LetterResize(MMDET_Resize):
    ...
    def _resize_bboxes(self, results: dict):
        ...
        # 추가) 0,0,0,0 인 경우 무시
        if isinstance(results['gt_bboxes'].tensor, torch.Tensor):
            if torch.all(results['gt_bboxes'].tensor == 0):
                return
        else:
            if np.all(results['gt_bboxes'].tensor == 0):
                return

        bboxes = results['gt_bboxes'].tensor

        #### 추가) 한 줄이라도 0,0,0,0 인 경우 face만 transform하고 나머지 무시
        # mask: 0,0,0,0 인 bbox 찾기
        if isinstance(bboxes, torch.Tensor):
            zero_mask = torch.all(bboxes == 0, dim=1)  # 각 행이 모두 0인지
            non_zero_mask = ~zero_mask
            # 0,0,0,0 제외한 bbox만 transform
            bboxes_to_resize = bboxes[non_zero_mask]
        else:
            zero_mask = np.all(bboxes == 0, axis=1)
            non_zero_mask = ~zero_mask
            bboxes_to_resize = bboxes[non_zero_mask]

        # 0,0,0,0 제외한 bbox만 transform 수행
        if len(bboxes_to_resize) > 0:
            results['gt_bboxes'].tensor = bboxes_to_resize
        ...
        # transform 끝난 bbox와 원래 0,0,0,0 bbox 합치기
        if isinstance(bboxes, torch.Tensor):
            results['gt_bboxes'].tensor = torch.cat([
                results['gt_bboxes'].tensor, bboxes[zero_mask]
            ], dim=0)

            if len(results['gt_bboxes'].tensor) != 3:
                print(results['gt_bboxes'].tensor)
        else:
            results['gt_bboxes'].tensor = np.vstack([
                results['gt_bboxes'].tensor, bboxes[zero_mask]
            ])
...
class YOLOv5HSVRandomAug(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # 변환된 H, S, V 채널을 하나의 이미지로 병합
        im_hsv = cv2.merge(
            (cv2.LUT(hue, lut_hue), cv2.LUT(sat,
                                            lut_sat), cv2.LUT(val, lut_val)))
        results['img'] = cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR)    # 변환된 HSV 이미지를 다시 BGR 색공간으로 변환하여 저장
        return results
...
class LoadAnnotations(MMDET_LoadAnnotations):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.mask2bbox:
            ...
            results = super().transform(results)               # 여기 안으로 들어가서 gaze 코드 추가 
            self._update_mask_ignore_data(results)
        return results
    ...
    # gt_gazes 추가
    def _load_gazes(self, results: dict):
        gt_gazes = []
        for instance in results.get('instances', []):
            gt_gazes.append(instance['gaze'])
        results['gt_gazes'] = np.array(gt_gazes, dtype=np.float32)
...
class YOLOv5RandomAffine(BaseTransform):
    ...
    def __init__(self,
                 ...
                 prob: float = 1.0,          # ***** 75% 확률 적용 ***** <- 기본 항상 적용이라 클래스 내부 수정.
                 ):
        ...
        self.prob = prob # ***** 75% 확률 적용 ***** <- 기본 항상 적용이라 클래스 내부 수정.
    ...
    def transform(self, results: dict) -> dict:
        ...
        # ***** 75% 확률 적용 ***** <- 기본 항상 적용이라 클래스 내부 수정.
        if random.random() < self.prob:
            return results  # 그대로 반환
...
@TRANSFORMS.register_module()
class CircularShift(BaseTransform):
    def __init__(self, axis='horizontal', num_parts=7, prob=0.5):
        self.axis = axis
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.axis == 'horizontal':
            part_w = w // self.num_parts
            shift_parts = random.randint(1, self.num_parts - 1)
            shift_px = shift_parts * part_w
            img_shifted = np.concatenate([img[:, shift_px:], img[:, :shift_px]], axis=1)
        else:
            part_h = h // self.num_parts
            shift_parts = random.randint(1, self.num_parts - 1)
            shift_px = shift_parts * part_h
            img_shifted = np.concatenate([img[shift_px:], img[:shift_px]], axis=0)
        ...
        # vis_dir = 'visualization/circular_shift'
        # os.makedirs(vis_dir, exist_ok=True)
        
        # img_orig = results['img'].copy()
        # vis_img = np.concatenate([img_orig, img_shifted], axis=1)
        
        # font = cv2.FONT_HERSHEY_SIMPLEX
        # cv2.putText(vis_img, 'Original', (img_orig.shape[1]//4, 30), font, 1, (0,255,0), 2)  # Center text over original image
        # cv2.putText(vis_img, 'Shifted', (img_orig.shape[1] + img_orig.shape[1]//4, 30), font, 1, (0,255,0), 2)  # Center text over shifted image
        
        # save_path = os.path.join(vis_dir, f'circular_shift_{self.axis}_{shift_parts}.jpg')
        # cv2.imwrite(save_path, vis_img)

        results['img'] = img_shifted
