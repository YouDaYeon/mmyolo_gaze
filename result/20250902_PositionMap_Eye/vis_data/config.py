...
last_transform = [
    dict(
        bbox_params=dict(
            ...
            type='BboxParams'),
        keymap=dict(gt_bboxes='bboxes', gt_gazes='gazes', img='image'),
        transforms=[
            dict(p=0.01, type='Blur'),
            dict(p=0.01, type='MedianBlur'),
            dict(p=0.01, type='ToGray'),
...
model = dict(
    ...
    gaze_head=dict(
        ...
        loss_gaze=dict(batch_size=32, loss_weight=6.0, type='GazeLoss_v0'),
        type='GazeHead_v0'),
    neck=dict(
        act_cfg=dict(inplace=True, type='SiLU'),
        deepen_factor=0.67,
...
pre_transform = [
    dict(backend_args=None, type='LoadImageFromFile'),
    dict(type='LoadAnnotations', with_bbox=True, with_gaze=True),
]
resume = False
...
test_dataloader = dict(
    ...
    dataset=dict(
        ...
        batch_shapes_cfg=None,
        data_prefix=dict(img='/mnt/terror/gaze360_dataset/original/imgs/'),
        data_root='/mnt/terror/gaze360_dataset/gaze_annotations/',
        metainfo=dict(
            classes=(
                'face',
...
test_evaluator = [
    dict(
        ann_file=
        '/mnt/terror/gaze360_dataset/gaze_annotations/gaze_test_all_bbox.json',
        classwise=True,
        metric='bbox',
        proposal_nums=(
            100,
...
val_evaluator = [
    dict(
        ann_file=
        '/mnt/terror/gaze360_dataset/gaze_annotations/gaze_val_all_bbox.json',
        type='AngularError'),
    dict(
        ann_file=
        '/mnt/terror/gaze360_dataset/gaze_annotations/gaze_val_all_bbox.json',
...
weight_decay = 0.0005
widen_factor = 0.5
work_dir = './work_dirs/yolov8_s_fast_1xb12-40e_gaze_v0'
